...
static uint16_t GetBME280Humidity(void);
 ...
 * @param[in] : None
 *
 * @param[out] : uint16_t - Humidity value read from BME280.
 *
 * @return : None
 *
 *****************************************************************************/
static uint16_t GetBME280Humidity(void)
{
    		...
    		&reg, 1, 100))
    {
        		...
        		ucHumideVal, 2, 100))
        {
            ...
            uiResult = (((uint16_t)ucHumideVal[0] << 8) |
                        ((uint16_t)ucHumideVal[1]));
        }
    }
...
static uint32_t GetBME280Temperature(void)
{
    		...
    		&reg, 1, 100))
    {
        ...
        {
            /* Assemble 20-bit raw pressure */
            uiResult = ((uint32_t)ucTempVal[0] << 12) |
                       ((uint32_t)ucTempVal[1] << 4)  |
                       ((ucTempVal[2] >> 4) & 0x0F);
        }
    }
